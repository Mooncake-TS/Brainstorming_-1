...
# Altair(면적 그래프). 설치 안 되어도 페이지가 "중간에서 멈추지" 않게 처리함.
try:
...
- 판매량과 재고량의 차이는 <b>생산으로 커버 가능한 수준</b>으로 설정
</div>
""",
        unsafe_allow_html=True,
    )
    # bin 총 재고는 대략 450~750 수준이므로 임계치도 그 안에서
...
def make_dummy_gap_data(n_bins=24, seed=7):
    """
    현실 반영:
    - ERP >= 실사 (실사 손실/불량/샘플 등 반영)
    - Gap 과도하지 않게: ERP가 실사보다 +20~+120 정도 크도록
    ...
    erp_over = rng.integers(20, 121, size=n_bins)      # 20~120
...
def apply_vision_rule(df: pd.DataFrame, use_inner_rule: bool):
    ...
    df["gap"] = df["vision_total"] - df["erp_stock"]   # 보통 음수(실사가 작음)
...
# ----------------------------
# Tab 2: Auto Gap Report
# ----------------------------
with tab2:
    st.markdown("### Auto Gap Report (ERP vs 실사 격차 자동 분석)")
    st.caption("격차(Gap)가 크거나(절대값), 실사 총량이 임계치(min) 미만인 bin을 우선순위로 표시합니다.")
...
with tab3:
    ...
    # 3년 평균 월 판매량(선택 월)
    sales_3y = sales_3y.copy()
    sales_3y["m"] = sales_3y["month"].dt.month
    month_sales = sales_3y[sales_3y["m"] == month_sel]
    ...
    # 3개 지표 long-form (그래프용)
    long = plan.melt(
        id_vars=["sku_id"],
        value_vars=["avg_sales", "vision_stock", "erp_stock"],
        var_name="metric",
        value_name="value"
    )
    ...
    st.markdown("#### 1) 면적 그래프(1장): 평균 판매량 vs 재고(실사/ERP)")
    if alt is None:
        st.warning("면적그래프(Altair)가 설치되어 있지 않아서, 임시로 표로 보여줄게. (설치: `pip install altair`)")
        ...
        chart = (
            ...
            .encode(
                ...
                color=alt.Color("metric_label:N", title="지표"),
                tooltip=[
